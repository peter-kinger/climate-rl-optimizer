...
def plot_colored_trajectory(
    csv_path, coord_cols=None, action_col=None, cmap_name="viridis", linewidth=2
):
    """
    ...
    xz, yz = np.meshgrid(np.linspace(x_min, x_max, 20), np.linspace(z_min, z_max, 20))
    yz, zz = np.meshgrid(np.linspace(y_min, y_max, 20), np.linspace(z_min, z_max, 20))
    ...
    legend_elements = [
        plt.Line2D(
            [0], [0], color=cmap(norm(action)), label=f"Action {action}", linewidth=2
        )
...
if __name__ == "__main__":
    ...
    plot_colored_trajectory(
        f"output/sparse/rl_model_DQN_network_dict_pi_vf_default_800000/episode_0_results_20250430_113455.csv",

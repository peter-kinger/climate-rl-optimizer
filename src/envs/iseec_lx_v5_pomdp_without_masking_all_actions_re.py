...
class IEMEnv(gym.Env):
    def __init__(
        ...
        **kwargs
        ...
        self.reward_weights = {
            ...
            'reward_weight_three_obj_over_re': {
                # 'Ta': 10,
                # 'Ca': 10,
                're': 1,
                'time': 2099,
            ...
            'reward_weight_three_obj_over_re_delta': {
                # 'Ta': 10,
                # 'Ca': 10,
                're': 1,
                'time': 2099,
            ...
            'reward_weight_three_obj_over_re_rate': {
                're_rate': 100,
            },
            'reward_weight_three_obj_over_re_cost': {
                # 'Ta': 10,
                # 'Ca': 10,
                're': 5,
                'cost_of_action': 0.05,
    ...
    def normalized_energy_intensity(self, state=None):
        """Normalize the energy state variable energy_intensity"""
        # 根据理论范围确定最大为 1 ，最小为限制的 0.7
        return (state - 0.7) / (1 - 0.7) 
    ...
    def get_reward_function(self, reward_type):
        ...
        def reward_weight_three_obj_over_re():
            """计算归一化后状态的距离范数奖励, 单目标
            """
            w = self.reward_weights['reward_weight_three_obj_over_re']
            
            T_a, C_a, C_o, C_od, T_o, E21, E22, E23, E24, E12 = self.state
            
            reward = 0
            r_energy_intensity = 0

            # if self.t < w['time']:
            r_energy_intensity = w['re'] * self.normalized_energy_intensity(self.energy_MYadjusted18502100_total_plus_B3B_plus_ACE3[self.t-1850] / self.energy_MYbaseline18502100_total_formulated[self.t-1850])
            reward = (r_energy_intensity)
            # else: 
                # reward = r_energy_intensity + w[over] * sum(self.energy_MYbaseline18502100_total_formulated[-len(time):]/self.energy_MYadjusted18502100_total_plus_B3B_plus_ACE3[-len(time):])

            # 记录分维度奖励
            # self.state_history["reward_Ta"].append(w['Ta'] * delta_PB_Ta ) # 基于已有的 reward 已经可以进行计算了
            # self.state_history["reward_Ca"].append(w['Ca'] * delta_PB_Ca )
            # self.state_history["reward_distance"].append(r_energy)
            self.state_history["reward_extra1"].append(r_energy_intensity)
            
            # 可以记录里面的 r_ta、 r_ca 和 distance
            # self.reward_dim1 = w['Ta'] * delta_PB_Ta 
            # self.reward_dim2 = w['Ca'] * delta_PB_Ca 
            self.reward_dim3 = r_energy_intensity
            # 增加维度
            # self.reward_dim4 = w["over"] * delta_PB_Ta 
            # self.reward_dim5 = w["over"] * delta_PB_Ca
            # self.reward_dim6 = r_end_energy
        ...
        def reward_weight_three_obj_over_re_delta():
            """计算归一化后状态的距离范数奖励, 单目标
            """
            w = self.reward_weights['reward_weight_three_obj_over_re_delta']
            ...
            # if self.t < w['time']:
            delta_re = self.normalized_energy_intensity(self.energy_MYadjusted18502100_total_plus_B3B_plus_ACE3[self.t-1850] / self.energy_MYbaseline18502100_total_formulated[self.t-1850]) - self.normalized_energy_intensity(0.7) # 把基线值减掉，减去baseline
            r_energy_intensity = w['re'] * delta_re
            reward = (r_energy_intensity)
            # else: 
                # reward = r_energy_intensity + w[over] * sum(self.energy_MYbaseline18502100_total_formulated[-len(time):]/self.energy_MYadjusted18502100_total_plus_B3B_plus_ACE3[-len(time):])
        ...
        def reward_weight_three_obj_over_re_rate():
            """计算归一化后状态的距离范数奖励, 单目标
            """
            w = self.reward_weights['reward_weight_three_obj_over_re_rate']
            ...
            delta_re_rate = self.re_t - self.re_t_prev
            reward = w['re_rate'] * delta_re_rate

            # 记录分维度奖励
            self.state_history["reward_Ta"].append(self.re_t ) # 基于已有的 reward 已经可以进行计算了
            self.state_history["reward_Ca"].append(self.re_t_prev)
            # self.state_history["reward_distance"].append(r_energy)
            self.state_history["reward_extra1"].append(delta_re_rate)
            
            # 可以记录里面的 r_ta、 r_ca 和 distance
            self.reward_dim1 = self.re_t 
            self.reward_dim2 = self.re_t_prev
            self.reward_dim3 = delta_re_rate
            # 增加维度
            # self.reward_dim4 = w["over"] * delta_PB_Ta 
            # self.reward_dim5 = w["over"] * delta_PB_Ca
            # self.reward_dim6 = r_end_energy
        ...
        def reward_weight_three_obj_over_re_cost():
            """计算归一化后状态的距离范数奖励, 单目标
            """
            w = self.reward_weights['reward_weight_three_obj_over_re_cost']
            ...
            reward = (r_energy_intensity - Cost_action) 
            # else: 
                # reward = r_energy_intensity + w[over] * sum(self.energy_MYbaseline18502100_total_formulated[-len(time):]/self.energy_MYadjusted18502100_total_plus_B3B_plus_ACE3[-len(time):])
            ...
            # 可以记录里面的 r_ta、 r_ca 和 distance
            # self.reward_dim1 = w['Ta'] * delta_PB_Ta 
            self.reward_dim2 = r_energy_intensity
            self.reward_dim3 = Cost_action
            # 增加维度
            self.reward_dim4 = cost_of_action1 *  w['cost_of_action']
            self.reward_dim5 = cost_of_action2 *  w['cost_of_action']
            self.reward_dim6 = cost_of_action3 *  w['cost_of_action']
        ...
        elif reward_type == "weight_three_obj_over_re":
            return reward_weight_three_obj_over_re
        elif reward_type == "weight_three_obj_over_re_cost":
            return reward_weight_three_obj_over_re_cost
        elif reward_type == "weight_three_obj_over_re_delta":
            return reward_weight_three_obj_over_re_delta
        elif reward_type == "weight_three_obj_over_re_rate":
            return reward_weight_three_obj_over_re_rate
        else:
    ...
    def render(self, mode="human"):
        ...
        # 绘制归一化后的 energy_intensity
        axs[2].cla()
        axs[2].set_title("Energy intensity")
        # 列表一一相减，需要列表推导式
        axs[2].plot(time, [e_adjuested / e_all for e_adjuested, e_all in zip(energy_adjusted, energy_baseline)], label="E11", color='orange')
        axs[2].set_xlabel("Time")

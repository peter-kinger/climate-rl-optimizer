...
class AYS_Environment(Env):
    ...
    possible_test_cases = [
        [0.4949063922255394, 0.4859623171738628, 0.5],
        [0.42610779, 0.52056811, 0.5],
    ]
    ...
    def __init__(
        ...
        **kwargs
        ...
        """
        self.A_PB = self._compactification(
            ays.boundary_parameters["A_PB"], self.X_MID[0]
        )  # Planetary boundary: 0.5897
        self.Y_SF = self._compactification(
            ays.boundary_parameters["W_SF"], self.X_MID[1]
    ...
    def step(self, action: int):
        ...
        if self._arrived_at_final_state() or not self._inside_planetary_boundaries():
            done = True
    ...
    def _perform_step(self, action, next_t):
        ...
        traj_one_step = odeint(
            ays.AYS_rescaled_rhs,
            self.state,
            [self.t, next_t],
            args=parameter_list[0],
            mxstep=50000,
        )
    ...
    def get_reward_function(self, choice_of_reward):
        ...
        def reward_distance_PB(action=0):
            if self._inside_planetary_boundaries():
                reward = np.linalg.norm(self.state - self.PB)  # 默认就是 2 范数
            else:
                reward = 0.0
            return reward
        ...
        if choice_of_reward == "final_state":
            ...
            print(
                "ERROR! You have to choose a reward function!\n",
                "Available Reward functions for this environment are: PB, rel_share, survive, desirable_region!",
            )
            exit(1)
        else:
            print(
                "ERROR! The reward function you chose is not available! "
                + choice_of_reward
            )
            print_debug_info()
            sys.exit(1)

    def calculate_expected_final_reward(
        self,
    ):
        """
        Get the reward in the last state, expecting from now on always default.
        This is important since we do not want the agent to find trajectories that are close (!) to final state and stay there, since this would
        result in a higher total reward.
        """
        reward_final_state = self.reward_function()
        remaining_steps = self.max_steps - self.t
        discounted_future_reward = 0.0
    ...
    def current_state_region_StartPoint(self):
        ...
        while not self._inside_planetary_boundaries():
            ...
            self.state[0] = self.current_state[0] + np.random.uniform(
                low=-limit_start, high=limit_start
            )
            self.state[1] = self.current_state[1] + np.random.uniform(
                low=-limit_start, high=limit_start
            )
            self.state[2] = self.current_state[
                2
            ]  # + np.random.uniform(low=-limit_start, high=limit_start)
    ...
    def _get_parameters(self, action=0):
        ...
        parameter_list = [
            (
                self.beta_LG if action_tuple[0] else self.beta,
                self.eps,
                self.phi,
                self.rho,
    ...
    def plot_run(
        ...
        for state_action in learning_progress:
            ...
            parameter_list = self._get_parameters(action)
            traj_one_step = odeint(
                ays.AYS_rescaled_rhs, state, sim_time_step, args=parameter_list[0]
            )
            # Plot trajectory
    ...
    def plot_current_state_trajectories(self, ax3d):
        ...
        for action_number in range(len(self.action_space)):
            ...
            traj_one_step = odeint(
                ays.AYS_rescaled_rhs, self.current_state, time, args=parameter_list[0]
            )
            ax3d.plot3D(
                xs=traj_one_step[:, 0],
                ys=traj_one_step[:, 1],
    ...
    def test_Q_states(self):
        ...
        testpoints = [
            ...
            [
                ...
                0.4475774101441196,
                0.5494879542441825,
            ],  # From here on for knick to green FP
            ...
            [
                ...
                0.4048031241155815,
                0.418890921031026,
            ],  # From here on for knick to black FP
            ...
            [
                ...
                0.5742868996790442,
                0.9699317116062534,
            ],  # From here on region of the shelter
...
class noisy_partially_observable_AYS(AYS_Environment):
    ...
    def __init__(
        ...
        run_number=0,
        plot_progress=False,
        observables=dict(A=True, Y=True, S=True),
        noise_strength=0.00,
    ):

        AYS_Environment.__init__(
            self, t0, dt, reward_type, image_dir, run_number, plot_progress
        )
    ...
    def _add_noise(self, state):
        ...
        noisy_state[noisy_state > 1.0] = 1.0
        return noisy_state
...
class noisy_AYS(AYS_Environment):
    ...
    def inject_noise(self):
        ...
        self.rho = 2.0 * np.clip(np.random.normal(1, self.noise), 0.5, 1.5)

...
class RunningMeanStd:
    def __init__(self, shape):
        ...
        self.count = 1e-4  # 避免初始时除以0

    def update(self, x):
        """用新的数据批次更新均值和标准差"""
        # 确保输入 x 是一个批次维度，例如 (1, N)
        batch_mean = np.mean(x, axis=0)
        batch_var = np.var(x, axis=0)
        batch_count = x.shape[0]

        # Welford's online algorithm 变种，处理批量数据
        ...
        m2 = m_a + m_b + delta_mean**2 * self.count * batch_count / total_count
...
class IEMEnv(gym.Env):
    def __init__(
        ...
        **kwargs
        ...
        # 动态归一化
        self.ta_normalizer = RunningMeanStd(shape=(1,))
        self.ca_normalizer = RunningMeanStd(shape=(1,))
        self.re_normalizer = RunningMeanStd(shape=(1,))
    ...
    def normalized_re(self, state=None):
        ...
        return (state - 0.69) / (1 - 0.69)  # 防止出错，最低值就是 0.69
    ...
    def get_reward_function(self, reward_type):
        ...
        def reward_weight_three_obj():
            ...
            self.state_history["reward_re"].append(w['re'] * delta_re)  
            
            # 可以记录里面的 r_ta、 r_ca 和 distance
            self.reward_dim1 = w['Ta'] * delta_PB_Ta 
            self.reward_dim2 = w['Ca'] * delta_PB_Ca 
            self.reward_dim3 = w['re'] * delta_re
        ...
        def reward_weight_three_obj_over_same():
            ...
            # 获取 T_a 的动态归一化值
            normalized_Ta_dynamic = self.ta_normalizer.normalize(np.array([[T_a]]))[0][0]

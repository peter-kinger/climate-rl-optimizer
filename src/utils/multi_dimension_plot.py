...
def plot_hairy_lines(num, ax3d, env):
    ...
    for i in range(num):
        ...
        ax3d.plot3D(
            ...
            ys=traj[:, 1],
            zs=traj[:, 2],
            color=colorbottom if traj[-1, 2] < 0.5 else colortop,
            alpha=0.08,
        )
...
def plot_run(learning_progress, env, fig, axes, colour, fname=None):
    ...
    for state_action in learning_progress:
        ...
        traj_one_step = odeint(
            env.iseec_dynamics_v1_ste, state, sim_time_step, mxstep=50000
        )
...
def plot_current_state_trajectories(
    start_state=None,
    model=None,
    env=None,
    colour=None,
    ...
    fig, ax3d = plot_run(
        learning_progress, env=env, fig=fig, axes=ax3d, fname=fname, colour=colour
    )
